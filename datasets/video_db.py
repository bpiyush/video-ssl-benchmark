...
class VideoDataset(data.Dataset):
    ...
    def __getitem__(self, index, return_frame_indices=False):
        if self.mode == 'clip':
            try:
            ...
            indices = np.arange(0, sample["frames"].shape[1])
            if self.frame_sampling == "random":
                indices = np.sort(np.random.choice(indices, int(self.video_fps), replace=False))
                # frames = sample["frames"]
                # sample["frames"] = [frames[x] for x in indices]
            elif self.frame_sampling == "linspace":
                # indices = np.arange(0, len(sample["frames"]))
                # indices = np.linspace(0, len(indices) - 1, int(self.video_fps), dtype=int)
                indices = deterministic_sample_clip_indices(len(indices), int(self.video_fps))
                # print(indices)
                # frames = sample["frames"]
                # sample["frames"] = [frames[x] for x in indices]
            elif self.frame_sampling == "linspace_with_offset":
                # import ipdb; ipdb.set_trace()
                
                # indices = np.linspace(0, len(indices) - 1, int(self.video_fps), dtype=int)
                # last_frame = indices[-1]
                # diff = indices[1] - indices[0]
                # indices = indices + np.random.randint(diff, size=int(self.video_fps))
                # indices[indices >= last_frame] = last_frame
                
                indices = random_sample_clip_indices(len(indices), int(self.video_fps))
                # print(indices)

                # indices = indices[indices < len(indices)]
                # indices = indices[indices >= 0]
                # frames = sample["frames"]
                # sample["frames"] = [frames[x] for x in indices]
            else:
                raise ValueError("Unknown frame sampling method")

            sample["frames"] = sample["frames"][:, indices]
